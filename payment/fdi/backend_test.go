...
import (
	...
	"github.com/rugwirobaker/paypack-backend/infra/fdi"
...
func newBackend() payment.Backend {
	...
	bc := fdi.NewBackend(timeOut, opts)
	return bc
}
...
func TestPull(t *testing.T) {
	...
	for _, tc := range cases {
		...
		assert.Equal(t, tc.status, status, fmt.Sprintf("%s: expected %s got %s", tc.desc, tc.status, status))
	}
